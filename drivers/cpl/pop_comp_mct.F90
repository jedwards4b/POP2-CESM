...
!BOP
!
! !IROUTINE: pop_init_mct
!
! !INTERFACE:
  subroutine pop_init_mct( EClock, cdata_o, x2o_o, o2x_o, NLFilename )
...
!-----------------------------------------------------------------------
    ...
    if(iam == 0) then
        lbnum=1
        call memmon_dump_fort('memmon.out','pop_init_mct:start::',lbnum) 
    endif
#endif
...
!-----------------------------------------------------------------------
    ...
    if (     trim(starttype) == trim(seq_infodata_start_type_start)) then
       ...
       write(stdout,*) 'pop_comp_mct ERROR: unknown starttype'
       call exit_POP(sigAbort,' pop_comp_mct ERROR: unknown starttype')
    end if

    !TODO: check for consistency of pop runid and runtype with seq_infodata
   
!   call seq_infodata_GetData( infodata, model_doi_url=model_doi_url) # BK: need newer CIME to call this
...
!-----------------------------------------------------------------------
   ...
   if (runtype == 'initial') then
      ...
      if (iyear0 /= start_year) then
         call document ('pop_init_mct', 'iyear0      ', iyear0)
         call document ('pop_init_mct', 'imonth0     ', imonth0)
         call document ('pop_init_mct', 'iday0       ', iday0)
         call document ('pop_init_mct', 'start_year  ', start_year)
         call document ('pop_init_mct', 'start_month ', start_month)
         call document ('pop_init_mct', 'start_day   ', start_day)
...
!-----------------------------------------------------------------------
   ...
   call init_time_flag('cpl_write_restart',cpl_write_restart, owner = 'pop_init_mct')
   call init_time_flag('cpl_write_history',cpl_write_history, owner = 'pop_init_mct')
   call init_time_flag('cpl_write_tavg'   ,cpl_write_tavg,    owner = 'pop_init_mct')
   call init_time_flag('cpl_diag_global'  ,cpl_diag_global,   owner = 'pop_init_mct')
   call init_time_flag('cpl_diag_transp'  ,cpl_diag_transp,   owner = 'pop_init_mct')
...
#if (defined _MEMTRACE)
    if(iam  == 0) then
!        write(6,*) 'pop_init_mct:end::'
        lbnum=1
        call memmon_dump_fort('memmon.out','pop_init_mct:end::',lbnum) 
...
!EOC
 ...
 end subroutine pop_init_mct

!***********************************************************************
!BOP
!
! !IROUTINE: pop_run_mct
!
! !INTERFACE:
  subroutine pop_run_mct( EClock, cdata_o, x2o_o, o2x_o)
!
! !DESCRIPTION:
...
!   call shr_file_getLogUnit (shrlogunit)  BK: ocpl manages log units
!   call shr_file_getLogLevel(shrloglev)   BK: ocpl manages log units
!   call shr_file_setLogUnit (stdout)      BK: ocpl manages log units
...
!----------------------------------------------------------------------------
    ...
    if (rstwr) then
       ...
       write(message,'(6a)') 'driver requests restart file at eod  ',  &
            cyear,'/',cmonth,'/',cday
       call document ('pop_comp_mct(run):', message)
    endif
...
!  call shr_file_setLogUnit (shrlogunit)  BK: ocpl manages stdout/log units
!  call shr_file_setLogLevel(shrloglev)   BK: ocpl manages stdout/log units

   call timer_stop(timer_total)

   write(stdout,*) subname," EXIT ","model time = ",ymd,tod
...
!EOC
  ...
  end subroutine pop_run_mct

!***********************************************************************
!BOP
!
! !IROUTINE: pop_final_mct
!
! !INTERFACE:
  subroutine pop_final_mct( EClock, cdata_o, x2o_o, o2x_o)
!
! !DESCRIPTION:
! Finalize POP
!
...
!-----------------------------------------------------------------------
   ...
   integer (POP_i4) :: &
    ...
    character(*), parameter :: subname = "(pop_final_mct) "

!-------------------------------------------------------------------------------

   write(stdout,*) subname," ENTER"  ; call shr_sys_flush(stdout)
   call POP_Final(errorCode)
   write(stdout,*) subname," EXIT"   ; call shr_sys_flush(stdout)

  end subroutine pop_final_mct
